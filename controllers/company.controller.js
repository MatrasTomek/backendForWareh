...
exports.postCompany = (req, res, next) => {
  // >>> data from Front <<<
  const { uzyt_id, pod_nazwa, pod_nip, pod_adres, pod_saldo } = req.body;
  ...
  promiseNipExist.then(() => {
    ...
    const promiseTopId = new Promise((resolve, reject) => {
      const sqlTopId =
        "SELECT pod_id FROM Podmiot ORDER BY pod_id DESC LIMIT 1";
    ...
    promiseTopId.then((pod_id) => {
      ...
      const promiseTopId2 = new Promise((resolve, reject) => {
        let up_id = null;
        const sqlTopId =
          "SELECT up_id FROM Uzyt_Podmiot ORDER BY up_id DESC LIMIT 1";
        db.query(sqlTopId, function (err, data, fields) {
          if (!err) {
      ...
      promiseTopId2.then((up_id) => {
        ...
        const sqlAddUp =
          "INSERT INTO Uzyt_Podmiot (up_id, up_uzyt_id, up_pod_id) VALUES (?,?,?)";
