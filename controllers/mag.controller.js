		...
		if (!err) {
			data.forEach((item) => {
				const calculatedRange =
					Math.sqrt(
						Math.pow(Number(coordinates.lng) - Number(item.mag_wspolrzedne_gps_dl), 2) +
							Math.pow(Number(coordinates.lat) - Number(item.mag_wspolrzedne_gps_szer), 2),
					) * 73;
	...
	promiseTopId.then((mag_id) => {
		...
		db.query(sqlAddWareh, newWareh, (err, data) => {
			if (!err) {
				...
				const props = {
					title: "Rejestracja nowego magazynu - dane magazynu",
					infoBeforeLink: `Właśnie zarejestrowałeś nowy magazyn: ${mag_nazwa}, adres: ${mag_adres}, kod-pocztowy: ${mag_kodpocztowy}, miejscowość: ${mag_miejscowosc}, opis: ${mag_opis}, mail kontaktowy: ${mag_kontakt_email}`,
					link: "",
					additionalInfo: "Pozdrawiamy, twojemagazyny.pl",
					subject: "Rejestracja Magazynu",
					mailTo: `${login}`,
				};
				...
				const adminData = {
					mailFrom: `${login}`,
					content: `dodał nowy magazyn: ${mag_nazwa}, adres: ${mag_adres}, kod-pocztowy: ${mag_kodpocztowy}, miejscowość: ${mag_miejscowosc}, opis: ${mag_opis}, mail kontaktowy: ${mag_kontakt_email}`,
				};
